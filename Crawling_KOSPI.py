...
def market_crawling(kospi_data_dict, kosdaq_data_dict, company_dict, update=False):
    ...
    # high, low 입력
    for dates in list(reversed(sorted(list(kospi_data_dict.keys())))):
        # kospi_data_dict와 kosdaq_data_dict의 keys(date)는 같다 (같아야 한다)
        # 해당 date는 각 종목의 price_dict에서도 그대로 사용한다

        if kospi_data_dict[dates][4] == -1 or kosdaq_data_dict[dates][4] == -1 or update == True:
            ...
            for keys in company_dict.keys():
                ...
                info_list = company_dict[keys][0]  # [0]임 << [0][0] 인지 [0] 인지 체크
                price_dict = company_dict[keys][1]  # dictionary 형태임

                market_type = info_list[20]
                company_name = info_list[1]

                if price_dict[dates][8] == 'high':
                    if market_type == 'kospi':
                        kospi_high_num += 1
                    else:
                        kosdaq_high_num += 1
                if price_dict[dates][8] == 'low':
                    if market_type == 'kospi':
                        kospi_low_num += 1
                    else:
                        kosdaq_low_num += 1

            kospi_data_dict[dates][4] = kospi_high_num
            kospi_data_dict[dates][5] = kospi_low_num
            kosdaq_data_dict[dates][4] = kosdaq_high_num
            kosdaq_data_dict[dates][5] = kosdaq_low_num


        else:  # 이후 날짜는 이미 업데이트되어있음
            print("{} 날짜 거래량, 증감 개수 업데이트 확인. 나머지 날짜 계산 생략".format(dates))
            break
    ...
    for dates in list(reversed(sorted(list(kospi_data_dict.keys())))):
        ...
        if kospi_data_dict[dates][6] == -1 or kosdaq_data_dict[dates][6] == -1 or update == True:

            kospi_trvol_plus = 0
            kospi_trvol_minus = 0
            kosdaq_trvol_plus = 0
            kosdaq_trvol_minus = 0
            ...
            for keys in company_dict.keys():
                ...
                if price_dict[dates][9] >= 0:
                    if market_type == 'kospi':
                        kospi_trvol_plus += price_dict[dates][9]
                    else:
                        kosdaq_trvol_plus += price_dict[dates][9]
                if price_dict[dates][9] < 0:
                    if market_type == 'kospi':
                        kospi_trvol_minus += price_dict[dates][9]
                    else:
                        kosdaq_trvol_minus += price_dict[dates][9]

            kospi_data_dict[dates][6] = kospi_trvol_plus
            kospi_data_dict[dates][7] = kospi_trvol_minus
            kosdaq_data_dict[dates][6] = kosdaq_trvol_plus
            kosdaq_data_dict[dates][7] = kosdaq_trvol_minus
    ...
    while crawling_switch:
        try:  # 데이터가 없다면 errMsg : "데이터가 존재하지 않습니다." 가 뜬다. Try / Except로 다음 월로 넘겨야 함
            ...
            if yearmonth[:4] + '-' + yearmonth[4:] in kosis_dict.keys() and \
                    kosis_dict[yearmonth[:4] + '-' + yearmonth[4:]] == result_list:  # 데이터 있음
                break
...
async def dispersion_crawling_data(company_list, company_dict, update=None, limits=None):
    ...
    for i in range(len(machines)):
        ...
        tasks.append(loop.create_task(
            command_machines("company_divided_pickle_" + str(i), **active_machines_dict[machines[i]], update=update, limits=limits)))
...
async def command_machines(crawling_kospi_dispersion_file, host, port, username, password, update=None, limits=None):
    ...
    async with asyncssh.connect(host, port=port, username=username, password=password) as conn:
        ...
        result = await conn.run('python Crawling_KOSPI_Dispersion.py ' + str(update) + ' ' + str(limits), check=True)
...
def KOSPI_KOSDAQ_calculate(company_price_dict, KOSPI_data_dict, KOSDAQ_data_dict):
